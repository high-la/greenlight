...
func (app *application) writeJSON(w http.ResponseWriter, status int, data envelope, headers http.Header) error {
	...
	// Use the json.MarshalIndent() fun so that whitespace is added to the encoded
	...
	// Append a newline to make it easier to view terminal apps.
	js = append(js, '\n')
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	...
	if err != nil {
		...
		// Use the error.As() function to check wether the error has the type
		// *json.SyntaxError. If it does, then return a plain english error message
		// which includes the location of the problem.
		case errors.As(err, &syntaxError):
			return fmt.Errorf("body contains badly-formed JSON (at character %d)", syntaxError.Offset)

		// In some circumstances Decode() may also return an io.UnexpectedEOF error
		...
		// Use the error.As() function to check whether the error has the type
...
func (app *application) readInt(qs url.Values, key string, defaultValues int, v *validator.Validator) int {
	...
	// Try to convert the value to an int. If this faile, add an error message to the
